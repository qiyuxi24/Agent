...
- **唯一真相源 = 仓库根 `branding.json`**（`productName`/`identifier`/`appTitle`/`packageName`/`userAgent`）。
...
- **零硬编码**：所有 CSS 文件中的 `#xxxxxx` 和硬编码 `rgba()` 已全部替换为 CSS 变量引用。唯一含颜色值的文件是 `variables.css`。
- **TS 侧常量**：`src/tokens/theme.ts`，导出 `palette` / `semanticLight` / `tokensLight` / `tokensDark`，供需要 JS 直接操作颜色的场景（canvas/图表等）。
- **主题切换**：CSS 类 `.theme-light` / `.theme-dark` + `@media (prefers-color-scheme: dark)` 系统跟随。
- **新增 token**：语义色 `--color-success/danger/error/warning/info/brand/ide`；组件色 `--bg-hover/elevated`、`--success-bg/border`、`--danger-bg/border`、`--code-header-*`、`--text-on-accent` 等。
